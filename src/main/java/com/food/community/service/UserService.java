package com.food.community.service;
...
import com.food.community.util.*;
...
//        return userMapper.selectById(id);
    ...
    public Map<String ,Object> login(String username,String password,int expiredSeconds){
        ...
        loginTicket.setStatus(0);//0有效，1失效
...
//        return userMapper.updateHeader(userId, headerUrl);
    ...
    //忘记密码发送验证码
    public void wjPassword(String email,String yzm){
    ...
    public Map<String,Object> upwmjm(String email,String yzm,String czpassword,String redisKey){
        Map<String ,Object> map = new HashMap<>();
        if(email == null){
            map.put("cemailMsg","邮箱为空");
            return map;
        }
        User user = userMapper.selectByEmail(email);
        if(user == null){
            map.put("cemailMsg","没有该邮箱");
            return map;
        }


        if (!StringUtils.isNotBlank(yzm)){
            map.put("yzmMsg", "验证码为空");
            return map;
        }
        //String kaptcha = (String) stringRedisTemplate.opsForValue().get(redisKey);
        String kaptcha = (String) redisTemplate.opsForValue().get(redisKey);

        if (!yzm.equals(kaptcha)) {
            map.put("yzmMsg", "验证码不正确!");
            return map;
        }
        czpassword =CommunityUtil.md5(czpassword+user.getSalt());
        int yes =  userMapper.updatePassword(user.getId(),czpassword);
        if(yes != 1){
            map.put("cpassword", "设置密码失败");
            return map;
        }
